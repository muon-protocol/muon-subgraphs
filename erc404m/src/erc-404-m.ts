...
function loadOrCreateERC721Token(tokenId: string): ERC721Token {
  let token = ERC721Token.load(tokenId);
  if (!token) {
    token = new ERC721Token(tokenId);
    token.owner = Bytes.fromHexString(ADDRESS_ZERO);
    token.save();
  }
  return token;
}
...
export function handleERC721Transfer(event: ERC721TransferEvent): void {
  ...
  let token = loadOrCreateERC721Token(event.params.id.toString())
