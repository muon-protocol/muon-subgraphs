...
export class SpiritSwapFactory extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save SpiritSwapFactory entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save SpiritSwapFactory entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("SpiritSwapFactory", id.toString(), this);
    }
  }

  static load(id: string): SpiritSwapFactory | null {
    return changetype<SpiritSwapFactory | null>(
      store.get("SpiritSwapFactory", id)
    );
  }
