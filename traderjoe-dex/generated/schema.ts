...
export class JoeFactory extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save JoeFactory entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save JoeFactory entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("JoeFactory", id.toString(), this);
    }
  }

  static load(id: string): JoeFactory | null {
    return changetype<JoeFactory | null>(store.get("JoeFactory", id));
  }
