...
export class SpookySwapFactory extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save SpookySwapFactory entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save SpookySwapFactory entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("SpookySwapFactory", id.toString(), this);
    }
  }

  static load(id: string): SpookySwapFactory | null {
    return changetype<SpookySwapFactory | null>(
      store.get("SpookySwapFactory", id)
    );
  }
